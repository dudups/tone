...
@AllArgsConstructor
@ToString
public class SeqNumOrTitle implements Query {
    @NotNull
    @Size(min = 1)
    ...
    public QueryBuilder queryBuilder(Function<String, String> fieldConverter) {
        ...
        QueryBuilder contains = Contains.builder().field(CardField.TITLE).values(values).build().queryBuilder(fieldConverter);
