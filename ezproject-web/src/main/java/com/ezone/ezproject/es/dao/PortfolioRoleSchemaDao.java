...
@Slf4j
@AllArgsConstructor
public class PortfolioRoleSchemaDao extends AbstractEsDocDao<Long, PortfolioRoleSchema> {
    @Override
    protected String index() {
        return EsIndexUtil.indexForPortfolioRoleSchema();
    }

    @Override
    public void saveOrUpdate(Long id, PortfolioRoleSchema schema) throws IOException {
        setDocSourceYaml(id, schema.yaml());
    }

    public Map<Long, PortfolioRoleSchema> find(List<Long> ids, String... fields) throws IOException {
    ...
    @Override
    protected @NotNull Function<String, PortfolioRoleSchema> deserializer() {
        return s -> {
            try {
                return PortfolioRoleSchema.from(s);
