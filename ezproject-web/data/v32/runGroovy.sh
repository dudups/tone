...
#local response=$(curl -X POST -H "X-INTERNAL-AUTH-TIMESTAMP: $timestamp" -H "X-INTERNAL-AUTH-MD5: $auth" -H "Content-Type: text/plain" "$1/ezProject/api/system/groovy?name=$2" --data-binary @"$2")
...
# eg: sh runGroovy.sh "https://ezone-dev.work" InitCompanyRoleRank.groovy
# eg: sh runGroovy.sh "https://ezone-dev.work" InitCustomerRoleRank.groovy
