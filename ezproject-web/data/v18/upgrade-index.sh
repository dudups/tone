...
function index_name() {
  ...
  local index_name=$(curl -u "$ES_AUTH" "$1/$2/_alias")
...
function create_new_index() {
  ...
  if ! curl -u "$ES_AUTH" -X PUT -H "Content-Type: application/json" "$1/$2" -d @"$3"; then
    exit 1
  fi
  echo
}
...
function update_index_settings() {
  ...
  if ! curl -u "$ES_AUTH" -X PUT -H "Content-Type: application/json" "$1/$2/_settings" -d @"$3"; then
    exit 1
  fi
  echo
}
...
function re_index() {
  ...
  if ! curl -u "$ES_AUTH" -X POST -H "Content-Type: application/json" "$1/_reindex" -d \
    '{
      "source": {
        "index": "'"$2"'"
      },
...
function del_index() {
  ...
  if ! curl -u "$ES_AUTH" -X DELETE "$1/$2"; then
    exit 1
  fi
  echo
}
...
function alias_index() {
  ...
  if ! curl -u "$ES_AUTH" -X PUT "$1/$2/_alias/$3"; then
    exit 1
  fi
  echo
}
...
function upgrade_index() {
  ...
  echo 'alias to new index'
  if ! alias_index "$endpoint" "$new_index_name" "$index_name"; then
    exit 1
  fi

  echo 'enable refresh and replica for new index'
